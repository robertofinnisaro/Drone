...
def PID(roll,pitch,yaw,f):
	...
	#-------------------------------------------------------------------------------------------------------------------------------
	#This is the Heart of the PID algorithm. PID behaves more accurately, if it is sampled at regular intervals. 
	#You can change the sampleTime to whatever value is suitable for your plant.
	if(dTime >= sampleTime):
		#Kp*e(t)
	...
	motor_speed = int(input("motor speed	"))
	
	#Map the esc values to motor values
	br_motor_vel = ((esc_br - 1500)/25) + motor_speed #50
	bl_motor_vel = ((esc_bl - 1500)/25) + motor_speed #50
	fr_motor_vel = ((esc_fr - 1500)/25) + motor_speed #50
	fl_motor_vel = ((esc_fl - 1500)/25) + motor_speed #50
	#----------------------------------------------------------------------------------------------------------------------------------
	...
	if(err_roll > 0 and err_pitch > 0):
		fl_motor_vel = 51
		fr_motor_vel = 45
		bl_motor_vel = 51
		br_motor_vel = 51
	elif(err_roll > 0 and err_pitch < 0):
		fl_motor_vel = 45
		fr_motor_vel = 51
		bl_motor_vel = 51
		br_motor_vel = 51
	'''	
	#---------------------------------------------------------------------------------------------------------------------------------
	#Provide the motor velocities to the object 'f' that will now exit out of this function, and gets published to gazebo, 
	# providing velocities to each motor. Note that the sign here is +,-,+,- i.e CW, CCW, CW, CCW in gazebo model. 
	# Change view of gazebo model (by scrolling) such that the green line comes to your left, red line goes forward, and blue line goes upward. 
	# This is the convention that i refer to as "Gazebo model" incase you get confused.
